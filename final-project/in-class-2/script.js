...
Promise.all([dataPromise, metadataPromise, geojsonPromise])
    ...
    // group world pop by income group
    // join data with map, e.g. dictionary
    
    const metaDataMap = d3.map(metadata, function(d){
      return d.Code;
    });
    
    console.log(metaDataMap.get('USA')['Income Group']);
    
    data.forEach(function(d){
      if (metaDataMap.get(d.countryCode)) {
        // could also take all the properties
...
function pickDataByYear(dataByYearByCountry, year) {
    ...
    const nodesData = dataByYear.map(function(country){
      ...
      // Group by year then income group
      // filter ony the indicator we want
