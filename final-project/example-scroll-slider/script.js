...
const plot = svg.append('g').attr('transform', `translate(${margin.l}, ${margin.t})`);
...
//Import data
const dataPromise = d3.csv('../data/data-test.csv', parseData).then(function (rows) {
...
Promise.all([dataPromise, metadataPromise, geojsonPromise]).then(function ([data, metadata, geojson]) {
  console.log(data);
  ...
  const xExtent = d3.extent(indicatorsByYear, function (d) {
    return d[INDICATOR_X]
  });
  const yExtent = d3.extent(indicatorsByYear, function (d) {
    return d[INDICATOR_Y]
  });
  const sizeExtent = d3.extent(indicatorsByYear, function (d) {
    return d[INDICATOR_SIZE]
  });
  ...
  //Sliders
  svg.append('g').attr('class', 'slider-container').attr('transform', `translate(${margin.l},30)`).call(slider, w, [2015, 2016, 2017], function (value) {
...
function filterByYear(indicators, year) {
  ...
  indicatorsByYear = d3.nest().key(function (d) {
    return d.countryCode
  }).entries(indicatorsByYear).map(function (d) {
...
function slider(selection, w, values, callback) {
  ...
  selection.append('line').attr('class', 'slider-range').attr('x1', 0).attr('x2', w).attr('y1', HEIGHT / 2).attr('y2', HEIGHT / 2).style('stroke', '#ccc').style('stroke-width', '2px');
  selection.selectAll('.tick').data(values).enter().append('circle').attr('class', 'tick').attr('cx', function (d) {
    return scale(d)
  }).attr('cy', HEIGHT / 2).attr('r', HEIGHT / 2 - 1).style('fill', 'white').style('stroke', '#ccc').style('stroke-width', '2px');
  const handle = selection.append('circle').attr('r', HEIGHT / 2 + 3).style('fill', 'white').style('fill-opacity', 0.01).style('stroke', '#666').style('stroke-width', '2px').attr('cx', 0).attr('cy', HEIGHT / 2);
  //drag behavior
  const dragBehavior = d3.drag().on('start', function () {}).on('drag', handleDrag).on('end', handleDragEnd);
...
function drawChart(data, plot) {
  //Draw nodes
  const nodes = plot.selectAll('.node').data(data, function (d) {
    return d.key
  });
  const nodesEnter = nodes.enter().append('g').attr('class', 'node');
  nodesEnter.append('circle');
  nodesEnter.append('text').attr('text-anchor', 'middle');
  nodes.merge(nodesEnter).transition().attr('transform', function (d) {
    const x = scaleX(d[INDICATOR_X]);
    const y = scaleY(d[INDICATOR_Y]);
    return `translate(${x}, ${y})`
  });
  nodes.merge(nodesEnter).select('circle').style('fill-opacity', .1).style('stroke', '#333').style('stroke-width', '1.5px').transition().attr('r', function (d) {
    return scaleSize(d[INDICATOR_SIZE])
  });
  nodes.merge(nodesEnter).select('text').style('font-size', '10px').text(function (d) {
    if (scaleSize(d[INDICATOR_SIZE]) > 16) return d.key;
    return null;
  });
  //Draw axes
  const axisX = d3.axisBottom().scale(scaleX).tickSize(-h);
  const axisY = d3.axisLeft().scale(scaleY).tickSize(-w);
